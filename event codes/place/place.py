...
async def on_button_click(res):
        ...
        for i in range(5):
            for j in range(5):
                print(b[i][j])
                if b[i][j]==int(res.component.label):
                    image.paste(Image.new('RGB', (200,200), getcolor(db[res.component.label], "RGB")), (j*200,i*200))
                    image.save("C:/Users/User/Desktop/events/event codes/place/full.png")
                    db[res.component.label]=l[(l.index(db[res.component.label])+1)%len(l)]
                    print(db[res.component.label])
                    break
        await res.respond(type=7,files=discord.file("C:/Users/User/Desktop/events/event codes/place/full.png"), suppress=True)
@client.command()
async def test(ctx):
    count=0
    b=[]
